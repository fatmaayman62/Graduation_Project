...
function PharmacyComponent({data}) {
  const {counter,setCounter} = useContext(CounterShoppingList)
  
  return (
    <>
      <div className="xl:w-1/4 lg:w-1/3 sm:w-1/2 w-full p-2">
        <div className="box dark:bg-[#1D293D] dark:shadow-amber-50/12 rounded-2xl bg-[#FFFFFF] border-2 border-main_border dark:border-0 p-4 shadow-sm hover:shadow-xl">
          <div className="medicine_info flex gap-4 my-2">
            <figure>
              <img src={data?.image} className="sm:h-14 sm:w-14 h-12 w-12 rounded-full" alt="" />
            </figure>
            <div className="info">
          ...
          <div className="flex justify-between items-center py-2">
            <span className="sm:text-lg text-[16px] font-medium">{data?.medicine}</span>
            <span className="sm:text-[24px] text-[20px] font-bold main_gradient bg-clip-text text-transparent">
              EGP {data?.price}
            </span>
          </div>
          <p className="flex items-center gap-1 bg-gray-400 dark:bg-[#18181B] rounded-2xl w-fit sm:text-[12px] text-[10px] px-2 my-2 text-white">
            <span className="my-auto sm:text-[16px] text-[14px]">✓</span> In Stock: {data?.stock} units
          </p>
          ...
          <div className="flex justify-between sm:flex-row flex-col sm:gap-0 gap-2 my-2">
            ...
            <Button className="sm:w-fit w-full sm:text-[14px] text-[12px] bg-blue-500 text-white rounded-[8px] hover:shadow-md mx-1"   onClick={() => setCounter(prev => prev + 1)}>
              Order
            </Button>
          </div>
        </div>
